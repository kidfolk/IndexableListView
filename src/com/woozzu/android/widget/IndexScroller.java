...
public class IndexScroller {
  ...
  public void draw(Canvas canvas) {
    ...
    final Paint indexBarPaint = getIndexBarPaint();
    canvas.drawRoundRect(mIndexbarRect, 5 * mDensity, 5 * mDensity, indexBarPaint);

    if (mSections != null && mSections.length > 0) {
      // Preview is shown when mCurrentSection is set
      if (mCurrentSection >= 0) {
        final Paint previewPaint = getPreviewPaint();
        final Paint previewTextPaint = getPreviewTextPaint();

        float previewTextWidth = previewTextPaint.measureText(mSections[mCurrentSection]);
        float previewSize =
            2 * mPreviewPadding + previewTextPaint.descent() - previewTextPaint.ascent();
        RectF previewRect =
            new RectF((mListViewWidth - previewSize) / 2, (mListViewHeight - previewSize) / 2,
      ...
      final Paint indexPaint = getIndexPaint();
  ...
  private Paint getIndexBarPaint() {
    ...
    return indexbarPaint;
  }

  private Paint getIndexPaint() {
  ...
  private Paint getPreviewTextPaint() {
    previewTextPaint = new Paint();
    previewTextPaint.setColor(Color.WHITE);
    previewTextPaint.setAntiAlias(true);
    previewTextPaint.setTextSize(50 * mScaledDensity);
    return previewTextPaint;
  }
